...
The dataset consists of publicly available AI policy texts from 100 universities, sourced from [Scribbr's GenAI University Policies page](https://www.scribbr.com/ai-tools/chatgpt-university-policies/). The data has been downloaded and saved as a CSV file for analysis.
...
This section covers how to apply both supervised and unsupervised machine learning models to predict insights from text data. We demonstrate how these techniques can enhance understanding of institutional GenAI policies through prediction and clustering.
...
**Purpose**: Supervised machine learning with text data can categorize policies based on themes, such as academic integrity, instructor discretion, and student support. This classification helps in understanding how different institutions frame their GenAI policies.
...
-   **RQ1**: Can we accurately classify GenAI policy texts into categories like "Academic Integrity," "Instructor Discretion," and "Student Support"?
...
```{r, eval = FALSE}
   ...
   policy_data <- read.csv("university_GenAI_policies_labeled.csv")  # Assume dataset includes a "Theme" column
...
**Purpose**: Unsupervised machine learning techniques like clustering and topic modeling are used for identifying hidden patterns, themes, or groupings within text data without labeled categories. These models are useful for uncovering common themes in GenAI policy texts.

**Case**: We explore topics discussed in GenAI policies across universities to uncover the main themes, grouping policies into clusters based on their emphasis (e.g., academic integrity, student support, instructor discretion).

------------------------------------------------------------------------

##### Sample Research Questions

-   **RQ1**: What are the main themes or topics present in GenAI policy texts across universities?
...
```{r, eval = FALSE}
   # Load dataset
   policy_data <- read.csv("university_GenAI_policies.csv")
...
-   **RQ1**: What are the main themes or topics present in GenAI policy texts across universities?\
    The LDA model reveals three key topics: 1) academic integrity, 2) student support, and 3) instructor discretion. These topics represent the primary areas of concern and focus in university GenAI policies.
...
### 2.6.2 Sample Research Questions
