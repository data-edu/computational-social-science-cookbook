...
**Case**: Using the ChatGPT policy texts from 100 universities, we apply Latent Dirichlet
...
-   **RQ1**: What are the main themes or topics present in university ChatGPT policy texts?
-   **RQ2**: How are the identified topics distributed across the dataset?
...
gpt_dtm_sparse_k3 <- as(gpt_dtm, "matrix")

# Fit the LDA model
lda_model <- LDA(gpt_dtm_sparse_k3, k = 3, control = list(seed = 1234))
