...
The tidyverse is a collection of R packages designed for data science with a shared philosophy: code should be readable, consistent, and focused on the data transformations you are actually trying to accomplish [@Wickham2017R4DS; @Wickham2019Tidyverse]. If you have struggled with base R's sometimes cryptic syntax, the tidyverse may feel like a relief.

Most computational educational research workflows rely on tidyverse packages for data manipulation, visualization, and modeling. Learning this ecosystem means learning the tools your collaborators use, the examples you will find online, and the approaches that scale from quick exploration to publication-ready analysis [@Wickham2017R4DS; @Wickham2019Tidyverse].

## 3.2 What is in the Tidyverse?
...
When you load the tidyverse, you will see a message listing which packages were attached and any conflicts (functions from tidyverse packages that mask base R functions). The conflicts are normal and intentional—tidyverse functions are generally preferable for data science work.

## 3.4 Working with Real Data

Let us load actual data to see the tidyverse in action. We will use student assessment data from the Open University Learning Analytics Dataset (OULAD), which contains information about student demographics and course outcomes.
...
This is the dataset we will use in the examples below. It represents the kind of educational data you will work with in computational educational research: thousands of observations with both categorical and continuous variables.

## 3.5 Essential dplyr Functions

The dplyr package provides functions for the most common data manipulation tasks. Here are the ones you will use repeatedly.

### 3.5.1 filter() - Selecting Rows
...
### 3.5.3 mutate() - Creating or Modifying Columns
...
### 3.5.4 summarize() - Calculating Summaries
...
### 3.5.5 group_by() - Operations by Group
...
### 3.5.7 count() - Quick Frequency Tables
...
Here is a concrete example using the dplyr functions we just learned:
...
We will use the native pipe `|>` throughout this book because it is now built into R, but you will encounter `%>%` in older code and documentation. For practical purposes, they are interchangeable in most tidyverse workflows.
...
## 3.7 Data Visualization with ggplot2
...
### 3.7.2 Bar Charts - Categorical Data

Use `geom_bar()` to visualize counts of categorical variables:

```{r}
#| label: plot-bar-simple
#| fig-cap: "Figure 3.1. Distribution of student final results"
# Simple bar chart
students |>
...
```{r}
#| label: plot-bar-stacked
#| fig-cap: "Figure 3.2. Stacked bar chart showing final results by gender"
...
```{r}
#| label: plot-bar-dodge
#| fig-cap: "Figure 3.3. Side-by-side bar chart showing final results by gender"
# Side-by-side bars
students |>
  ggplot(aes(x = gender, fill = final_result)) +
...
```{r}
#| label: plot-histogram-overlay
#| fig-cap: "Figure 3.5. Overlaid histograms of scores by pass status"
...
### 3.7.4 Boxplots - Comparing Distributions

Use `geom_boxplot()` to compare distributions across groups:

```{r}
#| label: plot-boxplot
#| fig-cap: "Figure 3.6. Score distributions by final result"
...
```{r}
#| label: plot-boxplot-color
#| fig-cap: "Figure 3.7. Score distributions by final result with color"
# With colors
students |>
  filter(!is.na(mean_weighted_score)) |>
...
### 3.7.5 Scatter Plots - Relationships

Use `geom_point()` to explore relationships between two continuous variables:

```{r}
#| label: plot-scatter
#| fig-cap: "Figure 3.8. Registration timing versus mean weighted scores"
...
```{r}
#| label: plot-scatter-smooth
#| fig-cap: "Figure 3.9. Registration timing versus scores with trend line"
# Add a trend line
students |>
  filter(!is.na(mean_weighted_score)) |>
  ggplot(aes(x = date_registration, y = mean_weighted_score)) +
...
### 3.7.6 Improving Plots with Labels and Themes

Good plots need clear labels:

```{r}
#| label: plot-bar-polished
#| fig-cap: "Figure 3.10. Distribution of student outcomes"
students |>
  ggplot(aes(x = final_result)) +
...
### 3.7.7 Faceting - Small Multiples

Use faceting to create separate plots for different groups:

```{r}
#| label: plot-facet-wrap
#| fig-cap: "Figure 3.11. Final results by course module"
...
This creates a grid of small bar charts, one for each module. It is useful when you want to compare patterns across many categories.

```{r}
#| label: plot-facet-grid
#| fig-cap: "Figure 3.12. Score distributions by gender and disability status"
...
The subsequent chapters in this book use these functions extensively, and we will introduce additional tidyverse capabilities as they become relevant. You will continue learning by doing, working with real data and real research questions.
