...
Images are an underused data source in educational research, yet they capture dimensions of learning environments — physical space, body language, classroom artifacts, and nonverbal interaction — that text and numeric data cannot [@Rose2016VisualMethodologies]. Treating images as data opens new research questions: How does body language shape classroom discourse? How do physiological signals correspond to engagement or affect during learning? Work by @DMello2017Engagement illustrates how multimodal signals, including visual data, can reveal aspects of the learning experience that single-mode studies miss.

This chapter introduces how educational researchers can capture and analyze images using local large language models in R — combining the richness of visual data with the privacy protections that sensitive educational contexts require.
...
*For the purpose of simplicity, we will only analyze nine photos from a [folder](https://github.com/data-edu/computational-social-science-cookbook/tree/main/data/s5_images). The process for batch analysis can be increased to more photos.*

![Figure 10.1. Classroom photo example for local LLM image analysis workflow.](data/s5_images/primary-school-teacher-working-with-students-in-classroom-838559688.jpg){width="575"}

![Figure 10.2. Second classroom photo example for comparative image interpretation.](data/s5_images/35320686-2551789306.jpg){width="576"}
...
write.csv(results_clean, "outputs/image_classification_detection_results.csv", row.names = FALSE)
...
### 10.3.2.6 Results and Discussion:

The analysis of classroom photographs using the `{kuzco}` package yielded structured insights across three domains: instructional context (classification), observable features (recognition), and affective tone (sentiment). Below, we summarize preliminary findings from two sample images. Full results can be seen [here](https://github.com/data-edu/computational-social-science-cookbook/blob/main/outputs/image_classification_detection_results.csv).
...
This chapter introduced image analysis as a method for educational research, using the `{kuzco}` package to classify, recognize, and assess sentiment in classroom photographs through a local large language model. The case study demonstrated that LLM-based image processing can generate structured, interpretable data from visual sources — capturing dimensions of learning environments that text and numeric data alone cannot. As with the text-based workflows in Chapter 7, the local execution model ensures that sensitive visual data remain on the researcher's own device.